...
{ 
    ...
    public Seed _seed { get { return seed; } set { seed = value; } }

    public Crops _crops { get { return crops; } set { crops = value; } }
    ...
    public void ButtonDownUse()
    {
        ...
        if (item_panel.activeSelf == false)
        {
            ...
            {
                iteminfo._seed = seed;
                iteminfo.nametex.text = seed.GetItemName();
                iteminfo.subscripttex.text = seed.GetItemsubscript();
                iteminfo.itemimage = gameObject.GetComponent<Image>();
                item_panel.SetActive(true);
                iteminfo.cnt = cnt;
                gameObject.GetComponent<Button>().interactable = false;
            ...
            if (crops != null)
            {
                iteminfo._crops = crops;
                iteminfo.nametex.text = crops.GetItemName();
                iteminfo.subscripttex.text = crops.GetItemsubscript();
                iteminfo.itemimage = gameObject.GetComponent<Image>();
                item_panel.SetActive(true);
                iteminfo.cnt = cnt;
                gameObject.GetComponent<Button>().interactable = false;
